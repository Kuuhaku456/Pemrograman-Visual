...
Public Class Completed
    ...
    Private Sub data_anime_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles data_anime.CellContentClick
        ...
        If data_anime.Columns(e.ColumnIndex).Name = "btn_detail" Then
            detail_animevb.judul_txt.Text = data_anime.Rows(e.RowIndex).Cells("judul_completed").Value
            detail_animevb.studio_txt.Text = data_anime.Rows(e.RowIndex).Cells("studio_completed").Value
            detail_animevb.rating_txt.Text = data_anime.Rows(e.RowIndex).Cells("rating_completed").Value
            detail_animevb.tipe_txt.Text = data_anime.Rows(e.RowIndex).Cells("tipe_completed").Value
            detail_animevb.episode_txt.Text = data_anime.Rows(e.RowIndex).Cells("episode_completed").Value
            detail_animevb.genre_txt.Text = data_anime.Rows(e.RowIndex).Cells("genre_completed").Value
            detail_animevb.tanggal_rilis_txt.Text = data_anime.Rows(e.RowIndex).Cells("tanggal_rilis_completed").Value
            detail_animevb.sinopsis_txt.Text = data_anime.Rows(e.RowIndex).Cells("sinopsis_completed").Value
            detail_animevb.pb_anime.Image = Image.FromFile(data_anime.Rows(e.RowIndex).Cells("gambar_completed").Value)
    ...
    Private Sub data_anime_SelectionChanged(sender As Object, e As EventArgs) Handles data_anime.SelectionChanged
        ...
        If anime > 0 Then
            ...
            pb_completed.Image = Image.FromFile(data_anime.CurrentRow.Cells("gambar_completed").Value)
        End If
    End Sub
End Class